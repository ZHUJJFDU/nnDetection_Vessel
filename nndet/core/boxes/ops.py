...
def expand_boxes(boxes: Tensor, scale_factor: float = 1.2) -> Tensor:
    """
    扩大边界框的大小，保持中心点不变
    
    Args:
        boxes (Tensor): 边界框 (x1, y1, x2, y2, (z1, z2))[N, dim * 2]
        scale_factor (float): 扩展因子，默认为1.2（扩大20%）
    
    Returns:
        Tensor: 扩大后的边界框 [N, dim * 2]
    """
    if boxes.numel() == 0:
        return boxes
        
    # 计算中心点
    centers = box_center(boxes)  # [N, dims]
    
    # 计算当前尺寸
    sizes = box_size(boxes)  # [N, dims]
    
    # 计算新的尺寸
    new_sizes = sizes * scale_factor
    
    # 计算新的边界框
    expanded_boxes = boxes.clone()
    
    # 2D或3D边界框
    expanded_boxes[:, 0] = centers[:, 0] - new_sizes[:, 0] / 2
    expanded_boxes[:, 1] = centers[:, 1] - new_sizes[:, 1] / 2
    expanded_boxes[:, 2] = centers[:, 0] + new_sizes[:, 0] / 2
    expanded_boxes[:, 3] = centers[:, 1] + new_sizes[:, 1] / 2
    
    if boxes.shape[1] == 6:  # 3D边界框
        expanded_boxes[:, 4] = centers[:, 2] - new_sizes[:, 2] / 2
        expanded_boxes[:, 5] = centers[:, 2] + new_sizes[:, 2] / 2
