...
from nndet.core.boxes import box_center, clip_boxes_to_image, remove_small_boxes, box_iou, box_area, expand_boxes
...
class BoxEnsembler(BaseEnsembler):
    ...
    def process_ensemble(self, boxes: List[Tensor], probs: List[Tensor],
        ...
        # 使用配置中的扩展因子扩大边界框
        box_expansion_factor = self.parameters.get("box_expansion_factor", 1.2)
        boxes = expand_boxes(boxes, scale_factor=box_expansion_factor)
    ...
    def get_default_parameters(cls):
        ...
        Returns:
            Dict:
                ...
                `ensemble_remove_small_boxes`: minimum size of the box
                `ensemble_score_thresh`: minimum probability
                `box_expansion_factor`: 边界框扩展因子，用于扩大检测框
        """
        return {
            # single model
            "model_iou": 0.1,
