...
def env_guard(func):
    """
    环境变量检查装饰器，用于验证nnDetection所需的环境变量是否设置
    
    功能:
    - 检查'det_data'环境变量是否存在
    - 检查'det_models'环境变量是否存在
    - 检查'OMP_NUM_THREADS'环境变量是否存在
    - 检查'det_num_threads'环境变量是否存在（非强制）
    - 检查'det_verbose'环境变量是否存在（非强制）
    
    Args:
        func: 需要被装饰的函数
    
    Raises:
        RuntimeError: 当必需环境变量未设置时抛出
    """
...
def check_dataset_file(task_name: str):
    ...
    参数:
        task_name (str): 需要验证的任务标识符
    
    验证内容:
    1. 基础元数据完整性（任务名称、数据维度）
    2. 标签定义格式校验
    3. 模态信息格式校验
    4. 目标类有效性检查
    
    Raises:
        ValueError: 当数据集信息不符合规范时抛出
    """
...
def _check_itk_params(
    ...
    参数:
        img_seq (sitk.Image序列): 需要验证的图像序列
        paths (Path序列): 对应的文件路径（用于错误信息）
