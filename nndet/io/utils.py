...
def load_dataset(folder: Pathlike) -> dict:
    ...
    for c in case_identifiers:
        ...
        # 添加血管分割文件路径
        dataset[c]['vessel_file'] = str(folder / f"{c}.npy")
    return dataset
...
def load_dataset_id(folder: Pathlike) -> dict:
    ...
    for c in case_ids:
        ...
        dataset[c]['properties_file'] = str(folder / f"{c}.pkl")
        dataset[c]['boxes_file'] = str(folder / f"{c}_boxes.pkl")
        # 添加血管分割文件路径，检查文件是否存在，如果不存在则设为None
        # 血管分割文件的索引
        vessel_file = folder / f"{c}.npy"
        dataset[c]['vessel_file'] = str(vessel_file) if vessel_file.exists() else None
    return dataset
